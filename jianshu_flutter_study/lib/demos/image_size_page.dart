...
import 'package:jianshu_flutter_study/res/styles.dart';

class ImageSizePage extends StatefulWidget {
  ImageSizePage(this.title, {Key key}) : super(key: key);
  final String title;

  _ImageSizePageState createState() => _ImageSizePageState();
}
...
class _ImageSizePageState extends State<ImageSizePage> {
  ...
  Widget _buildItem(String imageInfo, {height = 50}) {
    return Container(
  ...
  void _loadCachedNetworkImage() async {
    Image image = Image(
      image: CachedNetworkImageProvider(netUrl2),
    );
